...
def test_device_flow_chat_and_billing(client: TestClient) -> None:
    ...
    assert device_payload["verification_uri_complete"].startswith("http://testserver/device")

    complete = client.post(
        f"/v1/device/complete?user_code={device_payload['user_code']}",
        json={"email": "founder@krud.ai", "name": "Krud Founder"},
    )
    ...
    assert me.json()["email"] == "founder@krud.ai"
    ...
    webhook = client.post(
        "/v1/billing/webhook",
        json={"email": "founder@krud.ai", "status": "active", "customer_id": "cus_mock"},
    )
